...
import { deriveScoreDelta } from "../utils/score.js";
...
commentaryRouter.post('/', async (req, res) => {
    ...
    try {
        ...
        const scoreDelta = deriveScoreDelta(match, result);
        ...
        if (scoreDelta) {
            [updatedMatch] = await db
                .update(matches)
                .set({
                    homeScore: (match.homeScore ?? 0) + scoreDelta.home,
                    awayScore: (match.awayScore ?? 0) + scoreDelta.away,
                })
        ...
        if (updatedMatch && res.app.locals.broadcastScoreUpdate) {
            res.app.locals.broadcastScoreUpdate(updatedMatch.id, {
